...
let SOCK: Socket<ServerToClientEvents, ClientToServerEvents> = io("/", {
  autoConnect: false,
});
...
export const useUserStore = defineStore("userStore", {
  ...
  actions: {
    init() {
      ...
      SOCK.on("connect", async function () {
        ...
        if (self.groupid !== "[undefined]" && self.userid !== "") {
          console.log("joinroom", self.groupid);
          SOCK.emit("joinRoom", {
        ...
        SOCK.emit(
          "getUserData",
          { userid: self.userid, groupid: self.groupid, name: self.name },
          (user) => {
            if (user) {
      ...
      SOCK.on("loadGroupData", (data) => {
        console.log("loadGroupData", data);
      ...
      SOCK.on("setShowResults", ({ groupid, chapter }) => {
        if (!self.showResults.includes(chapter)) {
          self.showResults.push(chapter);
        }
      });
      SOCK.on("setUnShowResults", ({ groupid, chapter }) => {
        if (self.showResults.includes(chapter)) {
          self.showResults.splice(self.showResults.indexOf(chapter), 1);
        }
      });
    ...
    async setAnswer({
      ...
      if (!(chapter in this.answers)) {
        this.answers[chapter] = [];
      }
      // set answer locally
    ...
    async reset() {
      ...
      if (sure) {
        const done = await asyncEmit("removeUser", {
          groupid: this.groupid,
          userid: this.userid,
        });
        if (done) {
    ...
    async createUser({ name, userid }: { name: string; userid: string }) {
      const self = this;
      self.creating = true;
      const done = await asyncEmit("createUser", {
        userid,
        groupid: this.groupid,
        name,
      });
