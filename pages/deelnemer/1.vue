<template>
  <div class="user-chapter1" v-if="!user.loading">
    <userPause
      v-if="!started || (done && !user.showResults.includes('chapter1'))"
    ></userPause>
    ...
    >
      ...
      >
        ...
        <label
          class="jouw-antwoord"
          v-if="user.showResults.includes('chapter1')"
          >Jouw keuze:
          {{
        ...
        <div class="answers" v-if="user.showResults.includes('chapter1')">
          <BasicBar
            class="splitbar"
            v-if="allAnswers"
            :count="allAnswers.verwijderden[k].length"
            :total="
