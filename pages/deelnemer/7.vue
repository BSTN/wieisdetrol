<template>
  <div class="user-chapter-7">
    <div class="question">Kan jij de bot verslaan? Je mag zovaak proberen als je wilt, de beste reactie wordt automatisch jouw reactie en daarmee je score op het scorebord!</div>
    <div class="input">
    ...
    <div class="options" v-for="item in options">
      <div class="commentbox">
        <span>Score: {{ Math.round(item.score * 100) / 100 }}</span>{{ item.text }}
      </div>
    </div>
  </div>
</template>
...
async function send() {
  ...
  const data = await $fetch("http://localhost/beatthebot", {
    ...
    console.warn(err)
    error.value = 'Er is iets mis gegaan. Probeer nog een keer of probeer een andere tekst.'
    setTimeout(() => {
      error.value = ''
    }, 4000)
  });
  ...
  if (data !== undefined && data !== null) {
    ...
    const alltxt = JSON.parse(JSON.stringify(all.value))
    alltxt.sort((a,b) => b.score - a.score)
    user.setAnswer({chapter:'chapter7', k: 0, answer: {text: alltxt[0], score: data.score }})
    input.value = ''
  }
}
</script>
